...
Regarding introgression, we used data from [Vernot et al.](https://pubmed.ncbi.nlm.nih.gov/26989198/) and [Sankararaman et al.](https://pubmed.ncbi.nlm.nih.gov/27032491/). Both distributions lack the second peak *circa* 1,2mya characteristic of the [main distribution plots.](https://github.com/AGMAndirko/CLAP/blob/master/plots/KB19_distributions/3dist_sqrd.pdf), as expected. As these two studies seem to follow a slightly different distribution, we ran a KS test to see if that is the case and found that they are indeed significantly different (p val < 2.2e-16). 

We also extracted and dated those variants that are present in at least [one archaic](https://github.com/AGMAndirko/CLAP/blob/master/plots/KB19_distributions/filters/archaics_KB.pdf) in the Kuhlwilm & Boeckx (2019) database. In this case we found a bimodal distribution where the age distributions alleles from the three archaic individuals follow each other closely until the earlier distribution, where there's a clear contrast between the alleles shared with the two Neanderthal individuals (Altai and Vindija) and those shared with the Denisovan individual. This could be a consequence of the different timing of introgression events, assuming no artifacts.
