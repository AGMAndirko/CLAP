...
We also extracted and dated those variants that are present in at least [one archaic](https://github.com/AGMAndirko/CLAP/blob/master/plots/KB19_distributions/filters/archaics_KB.pdf) in the Kuhlwilm & Boeckx (2019) database. In this case we found a bimodal distribution where the age distributions alleles from the three archaic individuals follows each other closely, until the second distribution is reached, where there's a clear contrast in numbers between the alleles shared with the two Neanderthal individuals (Altai and Vindija) and those shared with the Denisovan individual. This could be a consequence of the different timing of introgression events.
